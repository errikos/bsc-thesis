...
If we now look at this procedure from an implementation point of view, it is only logical that exactly the same computation steps will be repeated, because \msf{B} is exactly the same as before. It seems inevitable that the new goal will produce exactly the same bindings as the previous one. And this is exactly the case, which is easily demonstrable if we look at the proof procedure computation steps of an example.
...
This time we make sure that our program also contains some facts, so that we can run \texttt{subset} against the facts' predicate. So, consider the following logic program:
\begin{lstlisting}[language=Prolog,frame=single]
  subset(P, Q) :- not(non_subset(P, Q)).
  non_subset(P, Q) :- P(X), not(Q(X)).
...
\begin{lstlisting}[language=Prolog,frame=single]
  ?- subset(P, q).
\end{lstlisting}
which asks for all subsets of \msf{q}. Since \msf{q} represents the set \msf{\left\{ 0, 1, 2 \right\}}, normally, 8 answers are expected and their order does not matter. Let us see what happens if we run the query by hand, highlighting the rules of the proof procedure that are applied at each step. In each step, we will show the pending goal list, as well as the unifications that have been produced up to that point of the computation.

The steps shown next are the steps that the current implementation follows, when given the above program and query. Please note that a left to right derivation order, just like in PROLOG, is followed.
...
\end{lstlisting}
The above is an application of the extended constructive negation rule for higher-order predicate variables.
...
\end{lstlisting}
Notice that the \texttt{V4} quantification is removed and a fresh variable is used.
...
Therefore, substitution \msf{\theta = \left\{ \texttt{P} / \left( \lambda \texttt{X}. (\texttt{X} = \texttt{V5}) \bigvee_{\kappa} \texttt{V8} \right) \right\}} is produced and the goal list is as follows:
\begin{lstlisting}[language=Prolog,%
  frame=single,breaklines=false,mathescape=true]
...
And after applying the unification:
\begin{lstlisting}[language=Prolog,%
  frame=single,breaklines=false,mathescape=true]
...
Furthermore, we reduce the first expression of the completed expression:
\begin{lstlisting}[language=Prolog,%
  frame=single,breaklines=false,mathescape=true]
...
The first negated expression is now a conjunction of expressions, the first of which is \texttt{false}, therefore the whole expression is \texttt{true}, thus the remaining goal list is:
\begin{lstlisting}[language=Prolog,%
  frame=single,breaklines=false,mathescape=true]
  $\sim$($\exists$V4 V8(V4), $\sim$q(V4)) ?
\end{lstlisting}
