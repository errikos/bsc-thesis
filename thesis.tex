  ...
  \item[7.] if \msf{A_i} is \msf{\left( R ~ \widehat{E} \right)} and \msf{R} is a predicate variable, then:
        \begin{enumerate}
          \item[(b)] if \msf{R \not \in \widehat{U}} and \msf{n > 1}, then
                ...
                where \msf{B_{1} = \sim\exists \widehat{U}_{1}' \left( S^{-}\left( R \left( \widehat{E} \right) \right) \land \left( A' \lor \left( \widehat{U}_{1} \approx_{\lor} \widehat{U}_{1}' \right) \right) \right)},

                \msf{\widehat{U}_1} are the variables in \msf{\widehat{U}} that are free in \msf{A_i}, \msf{\widehat{U}_2} are the variables in \msf{\widehat{U}} not in \msf{\widehat{U}_1} and \msf{\widehat{U}_{1}'} are renamed versions of variables in \msf{\widehat{U}_{1}};
        \end{enumerate}
