...
The first expression in the disjunction is the same, so it leads to the same first answer being returned, as before.
\begin{lstlisting}[language=Prolog,%
  frame=single,breaklines=false,mathescape=true,escapeinside={<@}{@>}]
   (4) $\sim$($\exists$V4 P(V4)) ?
...
At this point the implementation has to apply the trimmed substitution operator:
\begin{lstlisting}[language=Prolog,%
  frame=single,breaklines=false,mathescape=true,escapeinside={<@}{@>}]
...
Notice how at step 21 above, due to the negation, the equality in disjunction became an inequality in conjunction.
...
At this point the implementation will backtrack to step 24:
\begin{lstlisting}[language=Prolog,%
  frame=single,breaklines=false,mathescape=true,escapeinside={<@}{@>}]
...
At this point, rule \ref{def:negreduction}.6 is applied. The inequality \texttt{not(0 = 1)} is valid, therefore the first expression evaluates to \msf{true}.
\begin{lstlisting}[language=Prolog,%
  frame=single,breaklines=false,mathescape=true,escapeinside={<@}{@>}]
  (30) true,
       $\sim$($\exists$V13 S<@\textsuperscript{-}@>(($\lambda$X.X=1) $\vee_{\kappa}$ V17)(V13), ($\sim$q(V13); 0 = V13; 1 = V13)) ?
